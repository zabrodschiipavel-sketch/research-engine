...
сервера. Не проверялось программно (нужна ручная проверка после
перезапуска): что сам Claude Code/opencode при старте реально
подхватывает эти конфиги и видит инструменты в своём списке.
